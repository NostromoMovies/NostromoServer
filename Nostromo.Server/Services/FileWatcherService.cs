...
public class FileWatcherService : IHostedService, IDisposable
{
    ...
    private void OnFileDeleted(object? sender, string filePath)
    {
        ...
        if (_scheduler != null)
        {
            var jobKey = new JobKey($"ConsolidateJob_{filePath}", "ConsolidateGroup");
    ...
    private async Task ProcessFileAsync(string filePath)
    {
        ...
        while (true)
        {
            try
            {
                ...
                var jobKey = new JobKey($"ConsolidateJob_{filePath}", "ConsolidateGroup");
                var triggerKey = new TriggerKey($"ConsolidateTrigger_{filePath}", "ConsolidateGroup");

                // Create job detail
                var jobDetail = JobBuilder.Create<ConsolidateJob>()
                    .WithIdentity(jobKey)
                ...
                _logger.LogDebug("Successfully scheduled ConsolidateJob for {FilePath}", filePath);
